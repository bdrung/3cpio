...
impl Compression {
                ...
                Some(value) => {
                    if let Ok(level) = value.parse::<i64>() {
                        ...
                        if level >= min || level <= max {
                            ...
                            eprintln!("Compression level '{}' outside of range from {} to {}. Ignoring it.", level, min, max)
                        }
                    } else {
                        eprintln!(
                            "Unknown/unsupported compression parameter '{}'. Ignoring it.",
                            parameter
                        )
                    }
                }
    ...
    pub fn compress(&self, file: File, source_date_epoch: Option<u32>) -> Result<ChildStdin> {
