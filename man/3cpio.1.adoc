...
When generating initrd cpio archives, following manifest will be used.
...
fifo are also known as named pipe (see fifo(7))

*Limitations*: Files cannot start with # (will be treated as comment), be equal to - (will be treated as not specified), or contain tabs (will be split by tabs).
These limitations of the manifest file are not expected to cause problems in practice.
